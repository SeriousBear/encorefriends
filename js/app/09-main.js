function App() {
  ...
  const [showAdd, setShowAdd] = useState(false); // ➕ create action sheet
  ...
  // Which primary-nav destination is active, and the inbox unread badge count.
  const navSel = showNotifs
    ? "inbox"
    : view === "search"
      ? "discover"
      : view === "profile" && profileId === curUser.id
        ? "profile"
        : "feed";
  const navUnread =
    notifs.filter((n) => !n.read).length + unreadMsgCount + crewUnreadCount;
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {/* ── PRIMARY NAV — bottom bar (mobile) / left rail (desktop) ── */}
        <nav className="enav" aria-label="Primary">
          <button
            className={"enav-item" + (navSel === "feed" ? " on" : "")}
            onClick={() => {
              setShowNotifs(false);
              setView("feed");
            }}
          ...
          <button
            className={"enav-item" + (navSel === "discover" ? " on" : "")}
            onClick={() => {
              setShowNotifs(false);
              setView("search");
            }}
          ...
          <button
            ...
            onClick={() => (isGuest ? setShowAuth(true) : setShowAdd(true))}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3">
              <path d="M12 5v14M5 12h14" />
            </svg>
            <span className="enav-add-label">Add a show</span>
          </button>
          <button
            className={"enav-item" + (navSel === "inbox" ? " on" : "")}
            onClick={() => (isGuest ? setShowAuth(true) : openNotifs())}
          >
            <span className="enav-ic">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7">
                <path d="M21 15a2 2 0 0 1-2 2H8l-4 4V5a2 2 0 0 1 2-2h13a2 2 0 0 1 2 2z" />
              </svg>
              {navUnread > 0 && <span className="enav-dot" />}
            </span>
            <span>Inbox</span>
          </button>
          <button
            className={"enav-item" + (navSel === "profile" ? " on" : "")}
            onClick={() =>
              isGuest ? setShowAuth(true) : viewProfile(curUser.id)
            }
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7">
          ...
          >
            ...
            {FORWARDING_ENABLED && (
              ...
              >
                ...
                <span>
                  <span className="add-n">Forward a ticket</span>
                  <span className="add-d">Auto-tracks any show you bought</span>
                </span>
              </button>
            )}
            <button
            ...
            >
              ...
              <span>
                <span className="add-n">Add a show manually</span>
                <span className="add-d">No ticket yet — track it as interested</span>
              </span>
            </button>
          </div>
        </div>
